...
impl Cache<'_> {
    ...
    /// in the lockfile, and the registry tokens to authenticate the fetches with.
    pub async fn fetch_all(
        ...
        futures::stream::iter(sources)
            .map(|(entry, source)| async move {
                self.fetch_source(http_client, registry_tokens, entry, source)
                    .await
            })
    ...
    async fn fetch_source(
        ...
        match source {
            SourceWithIntegrity::Tgz { url, integrity } => {
                self.fetch_tgz_and_write_zip(client, registry_tokens, entry, url, integrity)
                    .await?
            }
    ...
    async fn fetch_tgz_and_write_zip(
        ...
        let file = fetch_to_tempfile(client, &url, registry_tokens).await?;
